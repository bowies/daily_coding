...
public class 홀수vs짝수_Java_230821{
    ...
    public int solution2(int[] num_list) {
        int 홀수 = IntStream.range(0, num_list.length)
                .filter(index -> index % 2 == 0) // 홀수 번째 원소 필터링
                .map(index -> num_list[index])
                .sum(); // 홀수 번째 원소의 합

        int 짝수 = IntStream.range(0, num_list.length)
                .filter(index -> index % 2 == 1) // 짝수 번째 원소 필터링
                .map(index -> num_list[index])
                .sum(); // 짝수 번째 원소의 합
